...
interface HeaderPathBarProps {
    ...
    styles: Pick<
        HeaderStyles,
        "pathSection" | "pathBar" | "toolbarTrigger" | "toolbarAction" | "toolbarDropdown"
    >;
}
                    ...
                    <div className="path-actions" role="group" aria-label="当前目录操作">
                        {selectedCount > 0 && (
                            <Dropdown
                                ...
                                classNames={{root: `${styles.toolbarDropdown} batch-dropdown`}}
                                menu={{items: batchMenuItems, onClick: handleBatchAction}}>
                                <button
                                    className={`${styles.toolbarTrigger} path-batch`}
                                    type="button"
                                    aria-haspopup="menu"
                                    aria-label={`批量操作，已选 ${selectedCount} 项`}>
                                    <Icon type="CheckSquareOutlined" className="marker"/>
                                    <span className="value">已选 {selectedCount} 项</span>
                        ...
                        {clipboardCount > 0 && (
                            <Button
                                className={`${styles.toolbarAction} path-paste`}
                                type="text"
                                ...
                                loading={pasting}
                                icon={<Icon type={clipboardMode === "move" ? "ScissorOutlined" : "SnippetsOutlined"}/>}
                                onClick={onPaste}>
                                <span className="value">
                                    {clipboardMode === "move" ? "移动" : "粘贴"} {clipboardCount} 项
                                </span>
                            </Button>
                        )}
                    </div>
