...
import FilePreviewPane from "@/pages/components/file-preview/media";
...
const previewKindLabels: Record<FilePreviewKind, string> = {
    image: "图片预览",
    video: "视频预览",
    audio: "音频预览",
};
...
const FileEditor = forwardRef(function FileEditor(
    ...
    const runAfterDiscard = useCallback((
        ...
        showConfirm();
    }, [destroyDiscardConfirm, exitWorkspaceFullscreen, files.hasUnsavedChanges, message, modal]);
    ...
    const openRenameTreeNode = useCallback((node: FileEditorTreeNode) => {
        ...
        const openForm = () => {
            ...
            formRef.current?.openRename(node.path, node.record, true);
        };
        if (document.fullscreenElement === workspaceRef.current) {
            void exitWorkspaceFullscreen()
                .then((exited) => exited && openForm())
    ...
    const previewKind = activeTab?.previewKind;
    const activeMode = getEditorMode(activeTab?.name || "");
    const currentAceError = !previewKind && aceError && activeTab && aceError.key === activeTab.key
        ? aceError.message
        : "";
    const displayError = activeTab?.error || currentAceError;
    const editorStatus = previewKind
        ? "只读"
        : displayError
        ? "加载失败"
        : activeTab?.loading
    ...
    const previewFile = useMemo(() => previewKind && activeTab ? ({
        path: activeTab.path,
        name: activeTab.name,
    }) : undefined, [activeTab?.name, activeTab?.path, previewKind]);
            ...
            <div
                ...
                <section className="file-editor-pane" aria-label="文件编辑区">
                    ...
                    <div id="file-editor-canvas" className="file-editor-canvas">
                        {activeTab && !previewKind && !activeTab.loading && !displayError && (
                            <MemoizedAceEditor
                        ...
                        {previewFile && !displayError && (
                            <FilePreviewPane
                                key={`${activeTab?.key}:${activeTab?.generation}`}
                                file={previewFile}
                                embedded
