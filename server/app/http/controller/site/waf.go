...
func Waf(c *context.Context) {
	var form struct {
		...
		Config *serviceSite.WAFUpdate `json:"config" validate:"omitempty" label:"WAF配置"`
	}
	...
	if form.Action == "get" {
		data, err := service.Site.GetWAF(form.Id)
		if err != nil {
			c.Error(err.Error())
		} else {
		...
		err := service.Site.UpdateWAF(form.Id, form.Config)
		if err != nil {
			c.Error(err.Error())
		} else {
