...
func Copy(c *server.Context) {
	...
	go func() {
		...
		service.Task.Update(taskID, task.StatusRunning, 0, "开始复制")
		_, _, _, err := f.Count(form.SourcePath)
		if err != nil {
			service.Task.Update(taskID, task.StatusFailed, 0, "计算文件大小失败: "+err.Error())
			return
		}
		err = f.CopyWithProcess(form.SourcePath, form.TargetPath, func(current, total int64, currentFile string, totalFiles, currentIndex int64) {
			select {
		...
		default:
			...
			if err != nil {
				service.Task.Update(taskID, task.StatusFailed, 0, "复制失败: "+err.Error())
				return
			}
